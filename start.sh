...
# priority 1: env var SSH_AUTHKEY (from Secret)
if [ -n "$SSH_AUTHKEY" ]; then
  echo "$SSH_AUTHKEY" > "$AUTH_KEY_FILE"
# priority 2: bundled authorized_keys file (repo)
elif [ -f "/app/authorized_keys" ]; then
  cp /app/authorized_keys "$AUTH_KEY_FILE"
else
  echo "⚠️ No SSH_AUTHKEY and no /app/authorized_keys found."
  echo "   You must provide a public key via env or repo file."
  touch "$AUTH_KEY_FILE"
fi
